<template>
  <div
    ...
    style="height: calc(100vh - var(--crm-topbar-h))"
  >
...
const leftWidth = ref(
  parseInt(localStorage.getItem("chat-left-width") || "320"),
);
const rightWidth = ref(
  parseInt(localStorage.getItem("chat-right-width") || "320"),
);
...
onMounted(async () => {
  ...
  // Nếu được navigate từ Contacts với ?convId=xxx, tự mở conversation đó
  const convId = route.query.convId as string | undefined;
  if (convId) {
    await selectConversation(convId);
    // Xoá query param khỏi URL để không bị re-trigger khi reload
...
<style scoped>
.chat-container {
  margin: -24px; /* compensate crm-page padding */
}
