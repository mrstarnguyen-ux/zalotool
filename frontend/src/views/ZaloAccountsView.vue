  ...
  <div>
      ...
      >
        ...
        <template #item.actions="{ item }">
          ...
          <v-btn
            ...
            size="small"
            color="orange"
            @click="openSyncHistory(item.id)"
            title="Đồng bộ lịch sử tin nhắn"
    ...
    <v-dialog v-model="showAddDialog" max-width="400">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" :loading="adding" @click="handleAddAccount"
            >Thêm</v-btn
          >
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="showQRDialog" max-width="400" persistent>
      <v-card class="text-center pa-4">
        ...
        <v-card-text>
          ...
          <v-alert v-if="qrError" type="error" density="compact" class="mt-2">{{
            qrError
          }}</v-alert>
        </v-card-text>
        <v-card-actions
          ><v-spacer /><v-btn @click="cancelQR">Đóng</v-btn></v-card-actions
        >
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="showDeleteDialog" max-width="400">
      <v-card>
        ...
        >
        <v-card-actions
          ><v-spacer /><v-btn @click="showDeleteDialog = false">Hủy</v-btn
          ><v-btn color="error" :loading="deleting" @click="handleDeleteAccount"
            >Xóa</v-btn
          ></v-card-actions
        >
      </v-card>
    ...
    <v-dialog v-model="showSyncHistoryDialog" max-width="480" persistent>
      <v-card>
        <v-card-title class="d-flex align-center"
          ><v-icon class="mr-2" color="orange">mdi-message-sync</v-icon>Đồng bộ
          lịch sử tin nhắn</v-card-title
        >
        <v-card-text>
          <div v-if="!syncHistoryRunning && !syncHistoryDone">
            <p class="mb-3">
              Tính năng này sẽ tải toàn bộ lịch sử tin nhắn từ Zalo về CRM.
            </p>
            <v-alert type="warning" density="compact" class="mb-3"
              >Quá trình có thể mất vài phút tùy số lượng hội thoại. Không tắt
              trình duyệt khi đang đồng bộ.</v-alert
            >
          </div>
          <div v-if="syncHistoryRunning">
            <p class="mb-2 font-weight-medium">{{ syncHistoryMsg }}</p>
            <v-progress-linear
              :model-value="syncHistoryPct"
              color="orange"
              height="12"
              rounded
              striped
            />
            <p class="text-caption mt-1 text-center">{{ syncHistoryPct }}%</p>
          </div>
          <div v-if="syncHistoryDone" class="text-center">
            <v-icon size="64" color="success">mdi-check-circle</v-icon>
            <p class="text-h6 mt-2">Hoàn thành!</p>
            <p class="text-body-2">{{ syncHistoryMsg }}</p>
          </div>
        </v-card-text>
        <v-card-actions
          ><v-spacer /><v-btn
            @click="closeSyncHistory"
            :disabled="syncHistoryRunning && !syncHistoryDone"
            >Đóng</v-btn
          ><v-btn
            v-if="!syncHistoryRunning && !syncHistoryDone"
            color="orange"
            :loading="syncHistoryRunning"
            @click="startSyncHistory"
            >Bắt đầu đồng bộ</v-btn
          ></v-card-actions
        >
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="showEditDialog" max-width="400">
      <v-card>
        ...
        <v-card-text>
          <p class="text-caption mb-4 text-grey">
            Tên này chỉ hiển thị trong hệ thống CRM, không thay đổi tên thật
            trên Zalo.
          </p>
          <v-text-field
            v-model="editName"
            label="Tên gợi nhớ mới"
            variant="outlined"
            hide-details
        ...
        <v-card-actions
          ><v-spacer /><v-btn variant="text" @click="showEditDialog = false"
            >Hủy</v-btn
          ><v-btn color="primary" :loading="updating" @click="handleUpdateName"
            >Lưu</v-btn
          ></v-card-actions
        >
      </v-card>
...
async function handleUpdateName() {
  ...
  updating.value = true;
  try {
    await api.patch(`/zalo-accounts/${editTarget.value.id}`, {
...
async function startSyncHistory() {
  ...
  syncHistoryPct.value = 0;
  if (!historySocket)
    historySocket = socketIO({ transports: ["websocket", "polling"] });
  historySocket.on(
    "history:progress",
    (data: { message: string; percent: number }) => {
      syncHistoryMsg.value = data.message;
      syncHistoryPct.value = data.percent;
    },
  );
  historySocket.on("history:done", (data: { result: any }) => {
    syncHistoryRunning.value = false;
    syncHistoryDone.value = true;
    const r = data.result;
  ...
  try {
    await api.post(
      `/zalo-accounts/${syncHistoryAccountId.value}/sync-history`,
      { socketId: historySocket.id },
    );
  } catch (err: any) {
    syncHistoryRunning.value = false;
