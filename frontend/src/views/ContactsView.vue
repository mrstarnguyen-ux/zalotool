<template>
  <div>
      ...
      <template #item.firstContactDate="{ item }">
        {{
          item.firstContactDate
            ? new Date(item.firstContactDate).toLocaleDateString("vi-VN")
            : "—"
        }}
      </template>
    ...
    <!-- Drawer: mở khi click vào KH hiện có (Info + Chat tabs) -->
    <ContactDrawer
      v-model="showDialog"
    ...
    <!-- Dialog: chỉ dùng khi tạo KH mới -->
    <ContactDetailDialog
...
import ContactDrawer from "@/components/contacts/ContactDrawer.vue";
...
const { contacts, total, loading, filters, pagination, fetchContacts } =
  useContacts();
...
const headers = [
  ...
  { title: "Từng đến", key: "nextAppointment", sortable: true },
