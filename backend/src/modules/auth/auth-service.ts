...
export interface JwtPayload { id: string; email: string; role: string; orgId: string; }
...
export async function setup(orgName: string, fullName: string, email: string, password: string): Promise<JwtPayload> {
  ...
  const result = await prisma.$transaction(async (tx: any) => {
    ...
    const ownerRole = await tx.role.create({
      data: {
        orgId: org.id, name: 'Chủ sở hữu', description: 'Quyền cao nhất',
    ...
    const memberRole = await tx.role.create({
      data: {
        orgId: org.id, name: 'Nhân viên', description: 'Quyền cơ bản',
    ...
    const user = await tx.user.create({
      data: { id: randomUUID(), orgId: org.id, email: email.toLowerCase().trim(), passwordHash, fullName, role: 'owner', roleId: ownerRole.id },
    });
    return { org, user };
  });

  logger.info(`Setup complete — org=${result.org.id}, user=${result.user.id}`);
  return { id: result.user.id, email: result.user.email, role: result.user.role, orgId: result.org.id };
}
...
export async function getProfile(userId: string) {
  const user = await prisma.user.findUnique({
    where: { id: userId },
    select: { id: true, email: true, fullName: true, role: true, orgId: true, teamId: true, isActive: true, createdAt: true, org: { select: { id: true, name: true } } },
  });
  if (!user) {
