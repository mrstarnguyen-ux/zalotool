...
export async function zaloRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/v1/zalo-accounts', async (request: any) => {
    ...
    if (dataScope === 'self') {
      // Thấy tài khoản do mình tạo HOẶC tài khoản được gán cho mình
    ...
    return accounts.map((a) => ({ ...a, liveStatus: zaloPool.getStatus(a.id) }));
  });

  app.post('/api/v1/zalo-accounts', async (request: any, reply: any) => {
    const user = request.user!;
  ...
  app.patch('/api/v1/zalo-accounts/:id', async (request: any, reply: any) => {
    const user = request.user!;
  ...
  app.post('/api/v1/zalo-accounts/:id/login', async (request: any, reply: any) => {
    const { id } = request.params;
    const user = request.user!;
    const account = await prisma.zaloAccount.findFirst({ where: { id, orgId: user.orgId } });
  ...
  app.post('/api/v1/zalo-accounts/:id/reconnect', async (request: any, reply: any) => {
    const { id } = request.params;
    const user = request.user!;
    const account = await prisma.zaloAccount.findFirst({ where: { id, orgId: user.orgId } });
    if (!account) return reply.status(404).send({ error: 'Account not found' });
  ...
  app.delete('/api/v1/zalo-accounts/:id', async (request: any, reply: any) => {
    const { id } = request.params;
    const user = request.user!;
    const account = await prisma.zaloAccount.findFirst({ where: { id, orgId: user.orgId } });
    if (!account) return reply.status(404).send({ error: 'Account not found' });
  ...
  app.get('/api/v1/zalo-accounts/:id/status', async (request: any, reply: any) => {
    const { id } = request.params;
    const user = request.user!;
    const account = await prisma.zaloAccount.findFirst({ where: { id, orgId: user.orgId } });
    if (!account) return reply.status(404).send({ error: 'Account not found' });
