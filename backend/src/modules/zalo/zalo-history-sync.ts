...
// Hàm chính
// ─────────────────────────────────────────────────────────────────────────────
export async function syncHistoryForAccount(
  accountId: string,
  api: any,
  orgId: string,
  io?: Server | null,
  ...
  const emitProgress = (msg: string, percent: number) => {
  ...
  const friendMap = new Map<string, { name: string; avatar: string }>();
  ...
  for (const friend of friends) {
    ...
    try {
      ...
      const { contactId, convId } = await ensureContactAndConversation(
        orgId,
        accountId,
        uid,
        friendInfo.name,
      ...
      while (page < MAX_PAGES) {
        if (Date.now() - threadStart > THREAD_TIMEOUT_MS) {
          logger.warn(
            `[history-sync] DM ${uid} timeout an toàn sau ${page} pages`,
          );
          break; // Quá 5 phút thì chủ động nhả ra
        }

        const msgs = await collectBatchWithRetry(
          api.listener,
          THREAD_TYPE_USER,
          lastMsgId,
          uid,
        );
        if (msgs.length === 0) break; // Thật sự hết tin nhắn
        ...
        if (!oldest || oldest === lastMsgId) break;
        lastMsgId = oldest;
        page++;
    ...
    dmDone++;
    emitProgress(
      `DM [${dmDone}/${friends.length}]`,
      Math.round(10 + (dmDone / (friends.length || 1)) * 20),
    );
  }
  ...
  for (const groupId of groupIds) {
    try {
      ...
      try {
        ...
        if (gInfo) {
          ...
          for (const mem of gInfo.currentMems || []) {
            ...
            if (memId && memName) memberNameMap.set(memId, memName);
          }
        }
      } catch (e) {}

      const combinedMap = new Map<string, string | { name: string }>([
      ...
      while (page < MAX_PAGES) {
        if (Date.now() - threadStart > THREAD_TIMEOUT_MS) break;
      ...
      groupDone++;
      emitProgress(
        `Nhóm [${groupDone}/${groupIds.length}] "${groupName}"`,
        Math.round(30 + (groupDone / (groupIds.length || 1)) * 65),
      );
    } catch (err) {
      result.errors++;
      groupDone++;
  ...
  emitProgress(
    `Hoàn thành! ${result.totalMessages} tin mới · ${result.skipped} đã có`,
    100,
  );
...
async function bulkSaveMessages(
  msgs: any[],
  convId: string,
  nameMap: Map<string, string | { name: string }>,
): Promise<number> {
  ...
  for (const msg of msgs) {
    ...
    if (!senderName && senderUid) {
      ...
      if (typeof entry === "string") senderName = entry;
      else if (entry && typeof entry === "object")
        senderName = (entry as any).name || "";
    }
...
async function collectBatchWithRetry(
  listener: any,
  threadType: number,
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      const msgs = await collectOneBatch(
        listener,
        threadType,
        lastMsgId,
        expectedThreadId,
...
function collectOneBatch(
  ...
  return new Promise((resolve, reject) => {
    ...
    const handler = (messages: any[], type: number) => {
...
function getOldestMsgId(msgs: any[]): string | null {
...
async function resolveAndBackfillNames(
  unknownUids: Set<string>,
  api: any,
  orgId: string,
) {
  for (const uid of unknownUids) {
    try {
      ...
      await prisma.$executeRawUnsafe(
        `UPDATE messages SET sender_name = $1 WHERE sender_uid = $2 AND (sender_name IS NULL OR sender_name = '')`,
        name,
        uid,
      );
    } catch (e) {}
