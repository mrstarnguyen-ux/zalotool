...
export function detectContentType(
  ...
  if (msgType.includes("recommended") || msgType.includes("card")) {
    ...
    if (parsed) {
      ...
      if (parsed.href?.includes("zalo.me/g/")) return "group_link";
      // Bubble ẩn (title = sendBubbleMessage)
      if (parsed.title === "sendBubbleMessage") return "bubble";
    }
    return "contact_card";
  }

  // FIX: bubble message (sendBubbleMessage qua msgType)
  if (msgType.includes("bubble") || msgType.includes("Bubble")) return "bubble";
...
function parseJsonContent(content: any): Record<string, any> | null {
 ...
 */
export function isRecallMessage(content: any): boolean {
 ...
 */
export function extractRecallMsgId(content: string): string | null {
  try {
