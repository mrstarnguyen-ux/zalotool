...
const { Zalo } = require("zca-js") as {
  Zalo: new (opts: { logging: boolean; selfListen?: boolean }) => any;
};
...
class ZaloAccountPool {
  ...
  async loginQR(accountId: string): Promise<void> {
    ...
    try {
        ...
        switch (event.type) {
          case 0: // QRCodeGenerated
            this.io
              ...
              .emit("zalo:qr", { accountId, qrImage: event.data.image });
            break;
          case 1: // QRCodeExpired
            this.io
              ?.to(`account:${accountId}`)
              .emit("zalo:qr-expired", { accountId });
            event.actions?.retry();
            break;
      ...
      prisma.zaloAccount
        .findUnique({ where: { id: accountId }, select: { orgId: true } })
        .then(
          (rec) =>
            rec && emitWebhook(rec.orgId, "zalo.connected", { accountId }),
        )
        .catch(() => {});
    } catch (err) {
      const instance = this.instances.get(accountId);
  ...
  private attachListener(accountId: string, api: any): void {
    // FIX: pass ownZaloUid so listener can correctly handle self-sent messages
    ...
    attachZaloListener({
      ...
      onDisconnected: (id) => {
        ...
        prisma.zaloAccount
          .findUnique({ where: { id }, select: { orgId: true } })
          .then(
            (rec) =>
              rec &&
              emitWebhook(rec.orgId, "zalo.disconnected", { accountId: id }),
          )
          .catch(() => {});
        ...
        const now = Date.now();
        const key = `dc_${id}`;
        const history = (this.disconnectHistory.get(key) || []).filter(
          (t) => now - t < 5 * 60_000,
        );
        history.push(now);
        ...
        if (history.length >= 5) {
          ...
          logger.error(
            `[zalo:${id}] Circuit breaker: ${history.length} disconnects in 5 min — stopping auto-reconnect. QR re-login required.`,
          );
  ...
  private saveCredentials(
    ...
    prisma.zaloAccount
      .update({
        where: { id: accountId },
        data: { sessionData: credentials as any },
      })
      .catch((err) =>
        logger.error(`[zalo:${accountId}] saveCredentials error:`, err),
      );
  }
  ...
  private async autoReconnect(accountId: string): Promise<void> {
    ...
    try {
      ...
      if (session?.imei) {
        ...
        logger.warn(
          `[zalo:${accountId}] No saved session, cannot auto-reconnect`,
        );
        this.io?.emit("zalo:reconnect-failed", {
          accountId,
