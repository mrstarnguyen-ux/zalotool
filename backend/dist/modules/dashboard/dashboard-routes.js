...
export async function dashboardRoutes(app) {
    ...
    app.get('/api/v1/dashboard/kpi', async (request, reply) => {
        try {
            ...
            const [messagesToday, unreplied, unread, aptsToday, newContacts, totalContacts] = await Promise.all([
    ...
    app.get('/api/v1/dashboard/pipeline', async (request, reply) => {
        try {
            ...
            return { data: pipeline.map((p) => ({ status: p.status, count: p._count })) }; // <--- ĐÃ SỬA DÒNG NÀY
        }
        catch (err) {
    ...
    app.get('/api/v1/dashboard/sources', async (request, reply) => {
        try {
            ...
            return { data: sources.map((s) => ({ source: s.source, count: s._count })) }; // <--- ĐÃ SỬA DÒNG NÀY
        }
        catch (err) {
    ...
    app.get('/api/v1/dashboard/appointments', async (request, reply) => {
        try {
            ...
            if (query.from || query.to) {
                ...
                if (query.from)
                    where.appointmentDate.gte = new Date(query.from);
                if (query.to)
                    where.appointmentDate.lte = new Date(query.to);
            }
            ...
            return { data: stats.map((s) => ({ status: s.status, count: s._count })) }; // <--- ĐÃ SỬA DÒNG NÀY
        }
        catch (err) {
