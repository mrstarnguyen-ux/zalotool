...
export async function roleRoutes(app) {
    ...
    app.post('/api/v1/roles', async (request, reply) => {
        ...
        if (!name)
            return reply.status(400).send({ error: 'Tên nhóm vai trò là bắt buộc' });
    ...
    app.put('/api/v1/roles/:id', async (request, reply) => {
        ...
        if (!existing)
            return reply.status(404).send({ error: 'Không tìm thấy nhóm vai trò' });
        if (existing.isSystem)
            return reply.status(403).send({ error: 'Không thể sửa quyền hệ thống mặc định' });
    ...
    app.delete('/api/v1/roles/:id', async (request, reply) => {
        ...
        if (!existing)
            return reply.status(404).send({ error: 'Không tìm thấy nhóm vai trò' });
        if (existing.isSystem)
            return reply.status(403).send({ error: 'Không thể xóa quyền hệ thống mặc định' });
