...
export async function orderRoutes(app) {
    ...
    async function generateOrderCode(orgId) {
    ...
    app.get('/api/v1/orders', async (request) => {
        ...
        const { page = '1', limit = '50', status = '', contactId = '', createdByUserId = '', } = request.query;
    ...
    app.put('/api/v1/orders/:id', async (request, reply) => {
    ...
    // Delete order
    app.delete('/api/v1/orders/:id', async (request) => {
        const { id } = request.params;
        await prisma.order.delete({ where: { id } });
        return { success: true };
    });
    // Orders for a specific contact
    app.get('/api/v1/contacts/:id/orders', async (request) => {
        const user = request.user;
        const { id } = request.params;
    ...
    app.get('/api/v1/orders/stats', async (request) => {
        const user = request.user;
    ...
    app.get('/api/v1/orders/by-staff', async (request) => {
        const user = request.user;
        ...
        const userIds = staffStats.map((s) => s.createdByUserId); // <--- ĐÃ SỬA DÒNG NÀY
        const users = await prisma.user.findMany({
            where: { id: { in: userIds } },
            select: { id: true, fullName: true },
        });
        const userMap = new Map(users.map((u) => [u.id, u.fullName])); // <--- ĐÃ SỬA DÒNG NÀY
