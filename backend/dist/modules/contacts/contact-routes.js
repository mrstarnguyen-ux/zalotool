...
export async function contactRoutes(app) {
    ...
    async function getContactScopeWhereClause(user) {
        ...
        if (dataScope === 'self') {
            ...
            if (currentUser?.teamId) {
                ...
                const zaloIds = teamMembers.map((m) => m.assignedZaloAccountId).filter(Boolean); // <--- ĐÃ SỬA DÒNG NÀY
    ...
    app.get('/api/v1/contacts/pipeline', async (request, reply) => {
        try {
            ...
            const statuses = pipeline.map((g) => g.status ?? 'unknown'); // <--- ĐÃ SỬA DÒNG NÀY
            const contactsByStatus = {};
            await Promise.all(statuses.map(async (st) => {
                const where = { orgId, status: st ?? null, ...scopeWhere };
    ...
    app.get('/api/v1/contacts/:id', async (request, reply) => {
        try {
            ...
            if (!contact)
                return reply.status(404).send({ error: 'Contact not found or access denied' });
            return contact;
        }
        catch (err) {
